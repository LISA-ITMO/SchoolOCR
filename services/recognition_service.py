...
class RecognitionService:
    ...
    def __init__(self, config_path: str = "config.json", api_keys_path: str = "api_keys.json", models_path: str = "./models"):
        ...
        self.code_recognizer = CodeRecognizer()
        self.table_recognizer = TableRecognizer(debug=False)  # Using the unified table recognizer
    ...
    @staticmethod
    def _load_models(path: str) -> Dict[str, Any]:
        """Load all required ML models."""
        model_paths = {
            "mnist": os.path.join(path, "mnist_model.keras"),
            "extended": os.path.join(path, "mnist_recognation_extendend.h5"),
            "yolo": os.path.join(path, "cell_detect.pt"),
            "yolo_extra": os.path.join(path, "cell_detect_extra.pt")
        }
        ...
        return {
            "mnist": tf.keras.models.load_model(model_paths["mnist"]),
            "extended": tf.keras.models.load_model(model_paths["extended"]),
            "yolo": YOLO(model_paths["yolo"]),
            "yolo_extra": YOLO(model_paths["yolo_extra"])
        }
    ...
    def recognize_image(self, image_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            # Table recognition
            task_numbers, digit_predictions = self._recognize_table(image)
    ...
    def _recognize_table(
        ...
        for yolo_model in [self.models["yolo"], self.models["yolo_extra"]]:
            scores = self.table_recognizer.recognize(
                image,
        ...
        if scores:
            tasks = [str(i) for i in range(1, len(scores) + 1)]
            return tasks, scores
