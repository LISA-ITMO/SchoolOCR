...
def analyze_json_files(folder_path):
    """
    Analyzes JSON files in a folder and its subfolders to categorize scores.

        Args:
            folder_path: The path to the folder containing the JSON files.

        Returns:
            tuple: A tuple containing a dictionary of score statistics
                   ('perfect', 'good', 'acceptable', 'poor') and the total number
                   of processed files.
    """
                    ...
                    with open(file_path, "r", encoding="utf-8") as f:
                        ...
                        if "scores" in data and isinstance(data["scores"], dict):
                            ...
                            for task, score_data in data["scores"].items():
                                if (
                                    isinstance(score_data, list)
                                    and len(score_data) >= 2
                                ):
                                    score = score_data[
                                        1
                                    ]  # Второй элемент - точность распознавания
                                    if score >= 0.9:
                                        stats["perfect"] += 1
                                    elif score >= 0.8:
                                        stats["good"] += 1
...
def plot_statistics(stats, total_files):
    """
    Plots a bar chart visualizing digit recognition statistics.

        Args:
            stats: A dictionary containing the counts for each recognition quality category
                   ('perfect', 'good', 'acceptable', 'poor').
