...
from utils.mnist_preprocess_code import (
    preprocess_image,
...
def process_and_recognize_digits(image_path):
    """
    Processes an image, recognizes digits within it using a pre-trained MNIST model, and returns the concatenated result.

        Args:
            image_path (str): The path to the input image file.

        Returns:
            str: A string representing the concatenated recognized digits from the image.
    """
    ...
    dilated = cv2.dilate(preprocessed, kernel=kernel, iterations=1)
...
if __name__ == "__main__":
    image_path = "to_proccess/cropped_code/page_1.jpg"
